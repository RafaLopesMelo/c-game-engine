...
#include <X11/XKBlib.h>   // sudo pacman -S libx11-dev
#include <X11/Xlib-xcb.h> // sudo pacman -S libxkbcommon-x11-dev
#include <X11/Xlib.h>
...
b8 platform_startup(platform_state *plat_state, const char *application_name,
    ...
    xcb_void_cookie_t cookie = xcb_create_window(
        state->connection, XCB_COPY_FROM_PARENT, state->window,
        state->screen->root, x, y, width, height, 0, // no border
    ...
    // tell server to notify if the window manager attempts to destroy the
    // window
...
b8 platform_pump_messages(platform_state *plat_state) {
    ...
    while (event != 0) {
        ...
        case XCB_BUTTON_RELEASE: {
            // mouse button key presses and releases
        } break;
...
void platform_console_write(const char *message, u8 colour) {
    const char *color_strings[] = {"0;41", "1;31", "1;33",
                                   "1;32", "1;34", "1;30"};
